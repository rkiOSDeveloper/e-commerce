...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tabDesc && tabShip && tabCancel && contentDesc && contentShip && contentCancel) {
        ...
        function setActiveTab(clickedTab) {
            tabs.forEach((tab, index) => {
                if (tab === clickedTab) {
                    ...
                    tab.classList.add('border-black', 'dark:border-white', 'font-bold', 'text-black', 'dark:text-white');
                    ...
                    tab.classList.remove('border-black', 'dark:border-white', 'font-bold', 'text-black', 'dark:text-white');
    ...
    sizeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            sizeBtns.forEach(b => {
                if (!b.disabled) {
                    b.className = 'size-btn py-3 border border-gray-300 dark:border-gray-600 rounded hover:border-black dark:hover:border-white text-sm font-medium transition-colors';
                }
            });

            // Set selected button
            btn.className = 'size-btn py-3 bg-black dark:bg-white text-white dark:text-black border border-black dark:border-white rounded text-sm font-bold shadow-md';
    ...
    thumbnails.forEach(thumb => {
        thumb.addEventListener('click', () => {
            ...
            thumb.classList.remove('border-transparent', 'hover:border-gray-300', 'dark:hover:border-gray-600');
    ...
    if (mainImage && imageModal && modalImage && closeModalBtn) {
        ...
        imageModal.addEventListener('click', (e) => {
            if (e.target === imageModal || e.target.closest('.relative') === imageModal.querySelector('.relative')) {
    ...
    if (shareBtn) {
        shareBtn.addEventListener('click', async () => {
            if (navigator.share) {
                ...
                alert('Sharing is not supported on this browser. Copying URL to clipboard is a good fallback.');
            }
        });
    }
    ...
    if (addToCartBtn) {
        addToCartBtn.addEventListener('click', () => {
            ...
            const activeColorBtn = document.querySelector('.color-button.border-black, .color-btn.border-black'); // Checks for active border
            if (activeColorBtn) {
            ...
            const product = {
                ...
                price: priceEl ? parseFloat(priceEl.textContent.replace('Rs. ', '').replace(/[^0-9.]/g, '')) : 0,
            ...
            if (typeof addToCart === 'function') {
                ...
                console.error('addToCart function not found. Ensure common.js is loaded.');
            }
        });
    }
});
