...
function updateGridButtons(view, count) {
  ...
  buttons.forEach(btn => {
    ...
    if (btnCount === count) {
      ...
      btn.classList.remove('text-gray-400', 'hover:text-black', 'dark:hover:text-white');
    } else {
      // Inactive State
      btn.classList.remove('bg-black', 'text-white', 'dark:bg-white', 'dark:text-black');
      btn.classList.add('text-gray-400', 'hover:text-black', 'dark:hover:text-white');
    }
  });
}
...
function selectSortOption(option, event) {
  ...
  links.forEach(link => {
    ...
    if (linkText === option) {
      link.classList.add('font-bold', 'text-gray-900', 'dark:text-white', 'bg-gray-50', 'dark:bg-gray-800');
      link.classList.remove('text-gray-700', 'dark:text-gray-300');
    } else {
      link.classList.remove('font-bold', 'text-gray-900', 'dark:text-white', 'bg-gray-50', 'dark:bg-gray-800');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loadMoreBtn && productGrid) {
    loadMoreBtn.addEventListener('click', () => {
      ...
      loadMoreBtn.innerHTML = '<span class="material-icons animate-spin mr-2 text-base">refresh</span> Loading...';
  ...
  const priceSlider = {
    ...
    init() {
      ...
      // Mobile
      this.mobileContainer = document.getElementById('mobile-price-slider-container');
      ...
      if (this.minThumb) {
        this.minThumb.addEventListener('mousedown', (e) => this.startDrag(e, 'min', this.container));
        this.minThumb.addEventListener('touchstart', (e) => this.startDrag(e, 'min', this.container), { passive: false });
      }
      if (this.maxThumb) {
        this.maxThumb.addEventListener('mousedown', (e) => this.startDrag(e, 'max', this.container));
        this.maxThumb.addEventListener('touchstart', (e) => this.startDrag(e, 'max', this.container), { passive: false });
      }
      if (this.minInput) this.minInput.addEventListener('change', (e) => this.handleInput(e, 'min'));
      if (this.maxInput) this.maxInput.addEventListener('change', (e) => this.handleInput(e, 'max'));

      // Attach Mobile Listeners
      if (this.mobileMinThumb) {
        this.mobileMinThumb.addEventListener('mousedown', (e) => this.startDrag(e, 'min', this.mobileContainer));
        this.mobileMinThumb.addEventListener('touchstart', (e) => this.startDrag(e, 'min', this.mobileContainer), { passive: false });
      }
      if (this.mobileMaxThumb) {
        this.mobileMaxThumb.addEventListener('mousedown', (e) => this.startDrag(e, 'max', this.mobileContainer));
        this.mobileMaxThumb.addEventListener('touchstart', (e) => this.startDrag(e, 'max', this.mobileContainer), { passive: false });
      }
      if (this.mobileMinInput) this.mobileMinInput.addEventListener('change', (e) => this.handleInput(e, 'min'));
      if (this.mobileMaxInput) this.mobileMaxInput.addEventListener('change', (e) => this.handleInput(e, 'max'));
    },
    ...
    updateUI() {
      ...
      // Update Mobile
      if (this.mobileMinThumb) this.mobileMinThumb.style.left = minPercent + '%';
      if (this.mobileMaxThumb) this.mobileMaxThumb.style.left = maxPercent + '%';
      if (this.mobileTrack) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (dropdown && sortButton && !dropdown.classList.contains('invisible') &&
